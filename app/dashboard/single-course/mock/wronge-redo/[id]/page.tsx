...
export default function WrongRedoPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      let redoIds: number[] = []
      try {
        const raw = sessionStorage.getItem("mock_redo_question_ids")
        if (raw) {
      ...
      try {
        // GET only — no progress / update / submit APIs
        ...
        const qData: { questions: ApiQuestion[] } = await qRes.json()
        if (dRes.ok) {
        ...
        const filtered = qData.questions
          .filter((q) => redoIds.includes(q.id))
          .map((q) => ({
            id: q.id,
            question: q.text,
  ...
  const finishPractice = () => {
    ...
    const items = questions.map((q, index) => {
      ...
      if (!isAnswered(q.selectedOption)) {
        skipped += 1
    ...
    const payload: WrongRedoResultPayload = {
      courseId,
      courseName:
    ...
    sessionStorage.setItem("mock_wrong_redo_result", JSON.stringify(payload))
    router.push("/dashboard/single-course/mock/wronge-redo/wronge-redo-result")
  }
