...
import type { WrongRedoResultPayload } from "../types"

const OPTION_LETTERS = ["A", "B", "C", "D"] as const

export default function WrongRedoResultPage() {
  const router = useRouter()
  const [payload, setPayload] = useState<WrongRedoResultPayload | null>(null)
  const [openIds, setOpenIds] = useState<Record<number, boolean>>({})
  const [ready, setReady] = useState(false)

  useEffect(() => {
    try {
      const raw = sessionStorage.getItem("mock_wrong_redo_result")
      if (raw) {
        setPayload(JSON.parse(raw) as WrongRedoResultPayload)
      }
    } catch {
      setPayload(null)
    } finally {
  ...
  const practiceAgain = () => {
    ...
    sessionStorage.setItem("mock_redo_question_ids", JSON.stringify(ids))
    router.push(
      `/dashboard/single-course/mock/wronge-redo/${payload.courseId}`
    )
  }
    ...
    <div className="min-h-screen bg-white px-4 py-12 text-black sm:px-6">
      <div className="mx-auto w-full max-w-2xl text-center">
        ...
        <p className="mt-5 text-sm text-neutral-500">
          This practice isn&apos;t saved. Take a full mock exam for official
          progress.
        </p>
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-2">
          ...
          <button
            ...
            onClick={() => {
              ...
              window.location.href = `/dashboard/single-course/mock/${payload.courseId}`
            }}
            className="inline-flex items-center justify-center gap-1.5 border border-black bg-white px-4 py-3 font-display text-sm font-semibold"
          >
            Take full mock exam
        ...
        <button
          ...
          onClick={() =>
            router.push(
              `/dashboard/single-course/mock/result/${payload.courseId}`
            )
          }
