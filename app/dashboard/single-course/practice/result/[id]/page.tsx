...
function buildProgressMap(progress: ProgressData) {
  const map = new Map<
    number,
    { selected_option: number | null; is_flagged: boolean }
  >()
  if (progress.questions && Array.isArray(progress.questions)) {
    for (const pq of progress.questions) {
      map.set(pq.question, {
        selected_option: pq.selected_option,
        is_flagged: pq.is_flagged,
      })
    }
  } else if (progress.chapters) {
...
export default function PracticeResultPage() {
  ...
  const [flatQuestions, setFlatQuestions] = useState<FlatQ[]>([])
    ...
    const load = async () => {
      ...
      try {
        const [qRes, aRes] = await Promise.all([
          ...
          fetch(
            `${process.env.NEXT_PUBLIC_BASE_URL}/quiz_progress/${courseId}/latest-submitted-analytics/`,
            { headers: { Authorization: token } }
          ),
        ])
        ...
        setProgress(wrapper.data)

        let domainIds: number[] | null = null
        try {
          const raw = sessionStorage.getItem("practice_domain_ids")
          if (raw) {
            const ids = JSON.parse(raw)
            if (Array.isArray(ids) && ids.length > 0) domainIds = ids
          }
        } catch {
          // ignore
        }
        ...
        for (const ch of qData.chapters || []) {
          if (domainIds && !domainIds.includes(ch.id)) continue
          for (const st of ch.subtopics || []) {
  ...
  const scored = useMemo(() => {
    if (!progress || flatQuestions.length === 0) {
      return {
        total: 0,
        correct: 0,
        incorrect: 0,
    ...
    const reviews: QuestionReview[] = flatQuestions.map((q, index) => {
      const pq = map.get(q.id)
      const selected = pq?.selected_option ?? null
    ...
    const domainMap = new Map<number, DomainStat>()
    for (const r of reviews) {
      const existing = domainMap.get(r.chapterId)
      if (!existing) {
        domainMap.set(r.chapterId, {
          id: r.chapterId,
          name: r.chapterName,
        ...
        existing.total += 1
        if (r.status === "correct") existing.correct += 1
      }
    }
    ...
    const finalCorrect = progress.correct_count ?? correct
    return {
      total,
  ...
  const redoEntire = () => {
    ...
    sessionStorage.removeItem("practice_wrong_redo_result")
    router.push(`/dashboard/single-course/practice/${courseId}`)
  }
  ...
  const redoWrong = () => {
    ...
    sessionStorage.setItem(
      "practice_redo_question_ids",
      JSON.stringify(scored.wrongIds)
    )
    router.push(
      `/dashboard/single-course/practice/wronge-redo/${courseId}`
    )
  }
    ...
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center gap-5 bg-white px-6 text-center">
        ...
        <p className="max-w-sm text-sm text-neutral-500">
          You haven&apos;t submitted a practice quiz for this course yet.
          Complete one to unlock your results here.
        </p>
  ...
  return (
    <div className="min-h-screen bg-white px-4 py-12 text-black sm:px-6">
      <div className="mx-auto w-full max-w-2xl text-center">
        <p className="font-mono text-[11px] font-semibold uppercase tracking-[0.16em] text-neutral-400">
          Practice quiz complete · {courseName || "Course"}
        </p>
                    ...
                    <div className="border-t border-neutral-100 bg-neutral-50 px-5 py-4">
                      ...
                      {item.explanation ? (
                        <div className="mt-4 border border-neutral-200 bg-white px-4 py-3">
                          <p className="font-mono text-[10px] font-semibold uppercase text-neutral-400">
                            Explanation
                          </p>
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-2">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-1.5 border border-black bg-white px-4 py-3 font-display text-sm font-semibold disabled:opacity-40"
          >
            Practice {scored.wrongIds.length || incorrectDisplay} wrong
            answers
            <ArrowRight className="h-3.5 w-3.5" />
          </button>
        </div>
        <p className="mt-2 text-center text-xs text-neutral-400">
          Wrong-answer practice is local only and does not update quiz
          progress. Use Redo entire quiz for an official attempt.
        </p>
