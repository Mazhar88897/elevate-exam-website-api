...
const Page = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store resetToken in sessionStorage
      if (data.resetToken) {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/auth/forgot-password`, {
        ...
        body: JSON.stringify({ email: sessionStorage.getItem('forgotPasswordEmail') }),
      });
      if (!response.ok) {
    ...
    <div className='m-5'>
      <div className="flex justify-center items-center flex-1 mt-12">
        <div className="w-full max-w-md border border-white rounded-mid p-6">
          ...
          <p className="text-white font-semibold text-sm ">Enter the 6-digit code sent to your email.</p>
          <p className="text-white text-xs  opacity-80 mb-6">If you do not see the email, please check your spam/junk folder.</p>
